...
module ROF
  RSpec.describe Access do
    context '.decode' do
      ...
      it 'handles multiple clauses with semicolon' do
        s = Access.decode('public;editgroup=group1,group2;edit=user2', 'user1')
        expect(s).to eq('read-groups' => ['public'], 'edit' => %w[user1 user2], 'edit-groups' => %w[group1 group2])
      end

      it 'handles multiple clauses with pipe' do
        s = Access.decode('public|editgroup=group1,group2|edit=user2', 'user1')
        expect(s).to eq('read-groups' => ['public'], 'edit' => %w[user1 user2], 'edit-groups' => %w[group1 group2])
      end
      ...
      it 'removes duplicates' do
        ...
        expect(s).to eq('edit' => %w[user1 user2])
      end
    end

    context '.encode' do
      it 'converts a Hash to a String' do
        s = Access.encode('read-groups' => ['public'], 'edit' => %w[user1 user2], 'edit-groups' => %w[group1 group2])
